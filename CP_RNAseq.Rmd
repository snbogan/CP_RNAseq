...
# Add abnormality variable by matching sample IDs
targets_gc$abnorm <- abnorm_df[match(colnames(featureCounts_df),
                                     abnorm_df$variable), 2]

abnorm <- targets_gc$abnorm
...
# Create model design that includes mat + dev effects, controls for abnorm, and sets intercept to 0
design_multi_gc <- model.matrix(~0 + Par + Dev + abnorm)
...
nrow(parental_QLFT_cutoff_df) # Without logFC cutoff = 3320 DEGs
nrow(parental_QLFT_fc_cutoff_df) # With logFC cutoff = 61 DEGs
...
# Get transcript lengths from spur5.0 gff
gff <- read.gff("~/Documents/GitHub/HotPurps_RNA/genome_data/spur5.0_GCF.gff")

# Prepare vs counts
transcript_ids <- rownames(featureCounts_df)
counts_m <- as.matrix(featureCounts_df)
rownames(counts_m) <- rownames(counts)
...
# Create transcript ID and gene ID variables
gff$transcript_id <- sub(
  ".*ID=([^;]+);Parent=gene.*",
  "\\1",
  gff$attributes
)

gff$gene_id <- str_extract(
  gff$attributes,
  "(?<=gene=)[^;]+"
)

# Keep only rows with transcript IDs
tx2gene <- unique(
  gff[
    !is.na(gff$transcript_id) &
      !is.na(gff$gene_id),
...
matched_gene_ids <- tx2gene$gene_id[
  match(rownames(counts_m), tx2gene$transcript_id)
]

# Remove transcripts without gene IDs
valid_tx <- !is.na(matched_gene_ids)

counts_m <- counts_m[valid_tx, ]
matched_gene_ids <- matched_gene_ids[valid_tx]
...
mean_cpm <- rowMeans(cpm_m)

# Create data frame for selecting highest CPM transcript per gene
tx_df <- data.frame(
  transcript_id = rownames(counts_m),
  gene_id = matched_gene_ids,
  mean_cpm = mean_cpm,
  stringsAsFactors = FALSE)

# Keep transcript with highest CPM for each gene
tx_keep <- tx_df %>%
  dplyr::group_by(gene_id) %>%
  dplyr::slice_max(mean_cpm, n = 1, with_ties = FALSE) %>%
  dplyr::ungroup()

# Subset counts matrix
counts_m <- counts_m[
  tx_keep$transcript_id,
]

# Replace transcript IDs with gene IDs
rownames(counts_m) <- tx_keep$gene_id
...
# Convert to kb
gene_lengths$gene_length_kb <- gene_lengths$gene_length / 1000

# Match lengths to counts matrix
length_kb <- gene_lengths$gene_length_kb[
  match(
    rownames(counts_m),
...
names(length_kb) <- rownames(counts_m)

# Remove genes without lengths
valid_len <- !is.na(length_kb)

counts_m <- counts_m[valid_len, ]
length_kb <- length_kb[valid_len]

# Calculate RPK
rpk <- counts_m / length_kb
...
# Filter matrix
tpm <- as.matrix(tpm_df[,-1])
storage.mode(tpm) <- "numeric"
rownames(tpm) <- tpm_df$gene_id

tpm_threshold <- quantile(tpm, probs = 0.05, na.rm = FALSE)

# Determine required number of samples
n_samples <- ncol(tpm)
min_samples <- ceiling(n_samples * 0.66)

# Count samples where gene >= threshold
keep <- rowSums(tpm >= tpm_threshold) >= min_samples

# Filter matrix
tpm_f <- tpm[keep, ]

# Report
cat("Original genes:", nrow(tpm), "\n")
cat("Filtered genes:", nrow(tpm_f), "\n")

# Mean-variance stabilize
tpm_f <- log2(1 + tpm_f)

# WGCNA
tpm_t <- t(tpm_f) # Transpose df

sft_pick <- pickSoftThreshold(tpm_t, powerVector = 1:20)
...
# Fit network
us_wgcna <- blockwiseModules(
  tpm_t, power = 3, maxBlockSize = 45000,
  TOMType = "unsigned", minModuleSize = 30, 
  deepSplit = 2, reassignThreshold = 0, 
  mergeCutHeight = 0.25, numericLabels = TRUE, 
  pamRespectsDendrogram = FALSE, saveTOMs = TRUE, 
  saveTOMFileBase = "~/Documents/Github/HotPurps_RNA/wgcna/us_tom", verbose = 3)

# Get module connectivities
us_ME <- us_wgcna$MEs
us_ME_colors <- labels2colors(us_wgcna$colors)

us_hubs <- as.data.frame(chooseTopHubInEachModule(tpm_t,
                                                  us_ME_colors,
                                                  power=10))

## Which modules associated with parental effect?
# Create dummy variables
par_matrix <- model.matrix(~ 0 + factor(targets_gc$Par))
colnames(par_matrix) <- levels(factor(targets_gc$Par))

# Correlation between MEs and traits
par_us_wgcna_cor <- cor(us_wgcna$MEs, par_matrix, use = "p")
par_us_wgcna_cor_abs <- abs(par_us_wgcna_cor)

# P-values for correlations
par_corr_pval <- corPvalueStudent(par_us_wgcna_cor, nSamples = nrow(par_matrix))

# Combine correlation and p-values
us_par_results <- as.data.frame(par_us_wgcna_cor)
us_par_pval_results <- as.data.frame(par_corr_pval)

## Identify most-associated modules
# Mean
par_us_mean_abs_cor <- apply(abs(par_us_wgcna_cor), 1, mean)
par_mean_MEs <- sort(par_us_mean_abs_cor, decreasing = TRUE)

# Max
par_max_abs_cor <- apply(abs(par_us_wgcna_cor), 1, max)
par_max_MEs <- sort(par_max_abs_cor, decreasing = TRUE)

# ME0 is the top-associated WGCNA module with parental treatment: r2 = 0.58932
# numeric module labels
module_numbers <- us_wgcna$colors

# corresponding colors
module_colors <- labels2colors(module_numbers)

# unique mapping
module_map <- unique(data.frame(
  module_number = module_numbers,
  module_color  = module_colors
))

module_map <- module_map[order(module_map$module_number), ]

# Turquoise has second highest correlation with maternal stress. Pink is number 1
module_map
...
tab_exp_df <- reshape2::melt(logCPM_df,
                   id = c("geneid"))
...
## SEMs with WGCNA data

```{r}

## Combine module eigengenes and morph data
morph_s <- morph_sum
morph_s$variable <- gsub("\\.", "", morph_s$variable)
us_ME$variable <- row.names(us_ME)

ME_morph_df <- merge(morph_s, us_ME, by = "variable")

# Parental treatment
ME_morph_df$par <- substr(ME_morph_df$variable, 1, 1)

# Developmental treatment
ME_morph_df$dev <- sub(".*_(.).*", "\\1", ME_morph_df$variable)

# Add abnormality data
ME_morph_df <- merge(ME_morph_df, abnorm_df, by = "variable")
...
# Identify ME columns
ME_cols <- names(ME_morph_df)[str_detect(names(ME_morph_df), "^ME")]

# Create derived and scaled variables
ME_morph_df <- ME_morph_df %>%
  mutate(
    Length_ratio = Length.spi / Length.bod,
    Length.bod_sc = as.numeric(scale(Length.bod)),
    Length_ratio_sc = as.numeric(scale(Length_ratio)),
    abnorm_sc = as.numeric(scale(PercAb))
  )

# Function to fit SEMs for a single ME column
run_sem_models <- function(me_col, df) {

  # Scale ME variable
  df[[paste0(me_col, "_sc")]] <- as.numeric(scale(df[[me_col]]))
  me_sc <- paste0(me_col, "_sc")
  ...
  model1 <- paste0(
    "
    # mediator
    ", me_sc, " ~ a*dev + par + abnorm_sc

    # outcome
    Length.bod_sc ~ cprime*dev + b*", me_sc, " + par + abnorm_sc
  ...
  fit1 <- sem(model1, data = df, missing = "fiml")

  params1 <- parameterEstimates(fit1, standardized = TRUE)

  indirect1 <- params1 %>%
    filter(label == "indirect")

  sig_mediation1 <- indirect1 %>%
    filter(pvalue < 0.05)

  ############################
  # MODEL 2: Length.spi / Length.bod
  ############################
  ...
  model2 <- paste0(
    ...
    # outcome
    Length_ratio_sc ~ cprime*dev + b*", me_sc, " + par + abnorm_sc
  ...
  fit2 <- sem(model2, data = df, missing = "fiml")
  ...
  list(
    ...
    fit_Length_bod = fit1,
    fit_Length_ratio = fit2,

    indirect_Length_bod = indirect1,
    indirect_Length_ratio = indirect2,

    sig_mediation_Length_bod = sig_mediation1,
    sig_mediation_Length_ratio = sig_mediation2
  )
}

# Run all SEMs
sem_results <- map(ME_cols, run_sem_models, df = ME_morph_df)

# Extract significant mediation results
sig_bod_results <- map_dfr(sem_results, function(x) {

  if(nrow(x$sig_mediation_Length_bod) > 0) {

    x$sig_mediation_Length_bod %>%
      mutate(
        ME = x$ME,
        outcome = "Length.bod"
      )
...
sig_ratio_results <- map_dfr(sem_results, function(x) {

  if(nrow(x$sig_mediation_Length_ratio) > 0) {

    x$sig_mediation_Length_ratio %>%
      mutate(
        ME = x$ME,
        outcome = "Length.spi/Length.bod"
      )
...
# Combine all significant mediation effects
sig_mediation_results <- bind_rows(
  sig_bod_results,
  sig_ratio_results
)

# View significant mediation effects
sig_mediation_results

# What module mediates dev effect on length? ME6
filter(sig_mediation_results, outcome == "Length.bod")$ME

# What module mediates dev effect on biomineralization? ME2
filter(sig_mediation_results, outcome == "Length.spi/Length.bod")$ME

# What modules are associated with length and biomin, controlling for abnormality?
# ME2, ME6, and ME69
library(broom)
...
for(me_col in ME_cols) {
  ...
  # Scale ME variable
  me_sc <- as.numeric(scale(ME_morph_df[[me_col]]))
  ...
  # MODEL A: Length.bod ~ ME + abnorm
  mod_length <- lm(
    Length.bod_sc ~ ME_sc + abnorm_sc,
    data = tmp_df
  )

  res_length <- tidy(mod_length) %>%
    filter(term == "ME_sc") %>%
    mutate(
      ME = me_col,
      outcome = "Length.bod"
    ...
    # MODEL B: Length.spi/Length.bod ~ ME + abnorm
  mod_ratio <- lm(
    Length_ratio_sc ~ ME_sc + abnorm_sc,
    data = tmp_df
  )

  res_ratio <- tidy(mod_ratio) %>%
    filter(term == "ME_sc") %>%
    mutate(
      ME = me_col,
      outcome = "Length.spi/Length.bod"
...
# Adjust p-values separately for each outcome
...
sig_corr_results <- corr_results_df %>%
  filter(p_adj < 0.05)
...
## Generate G matrix of ME2 and ME6

```{r}

# Make relatedness matrix (code repeats below)
...
# Buggy makeA matrix... use above
# relat_m <- as.matrix(makeA(pedi_p))

# Export ME2 genes: 2715 genes
ME2_genes <- rownames(filter(as.data.frame(us_wgcna$colors), `us_wgcna$colors` == 2))

# Export ME6 genes: 176 genes
ME6_genes <- rownames(filter(as.data.frame(us_wgcna$colors), `us_wgcna$colors` == 6))

# Are ME2 and ME6 eigengenes correlated? Yes... they are negatively correlated
ggplot(data = us_ME,
       aes(x = ME2, y = ME6)) +
  geom_point() +
...
### -----------------------------
### 1. Inverse relatedness matrix
### -----------------------------
Ainv <- solve(cstm_A_mat)
Ainv <- as(Ainv, "dgCMatrix")
rownames(Ainv) <- colnames(Ainv) <- rownames(cstm_A_mat)

### -----------------------------
### 2. Prepare data
### -----------------------------
us_ME$animal <- as.character(us_ME$variable)
us_ME$animal <- substr(us_ME$animal, 1, nchar(us_ME$animal) - 1)

dat <- us_ME[, c("animal", "ME2", "ME6")]
dat <- dat[complete.cases(dat), ]

dat <- dat[dat$animal %in% rownames(Ainv), ]
dat$animal <- factor(dat$animal, levels = rownames(Ainv))

### -----------------------------
### 3. Long format (NO reserved "trait")
### -----------------------------
dat_long <- reshape(
  dat,
  varying = c("ME2", "ME6"),
...
dat_long$trait_id <- factor(dat_long$trait_id)

# Create parental variable
dat_long$par <- substr(dat_long$animal, 
                       nchar(as.character(dat_long$animal)), 
                             nchar(as.character(dat_long$animal)))
...
### -----------------------------
prior <- list(
  G = list(G1 = list(V = diag(2), nu = 2.002)),
  R = list(V = diag(2), nu = 2.002)
)

### -----------------------------
### 5. Fit multivariate animal model
### -----------------------------
modN <- MCMCglmm(
  fixed = y ~ trait_id - 1,
  random = ~ us(trait_id):animal,
  rcov   = ~ us(trait_id):units,
  family = "gaussian",
  ginverse = list(animal = Ainv),
  data = filter(dat_long, par == "N"),
  prior = prior,
  nitt = 300000,
  burnin = 50000,
  thin = 100,
...
### -----------------------------
### 6. Extract posterior G matrix
### -----------------------------
VCVN <- modN$VCV
VCVU <- modU$VCV

G11N <- VCVN[, "trait_idME2:trait_idME2.animal"]
G22N <- VCVN[, "trait_idME6:trait_idME6.animal"]
G12N <- VCVN[, "trait_idME2:trait_idME6.animal"]

G11U <- VCVU[, "trait_idME2:trait_idME2.animal"]
G22U <- VCVU[, "trait_idME6:trait_idME6.animal"]
G12U <- VCVU[, "trait_idME2:trait_idME6.animal"]

G_meanU <- matrix(
  c(mean(G11U), mean(G12U),
    mean(G12U), mean(G22U)),
  nrow = 2, byrow = TRUE
)

G_meanN <- matrix(
  c(mean(G11N), mean(G12N),
    mean(G12N), mean(G22N)),
  nrow = 2, byrow = TRUE
)

colnames(G_meanU) <- rownames(G_meanU) <- c("ME2", "ME6")
colnames(G_meanN) <- rownames(G_meanN) <- c("ME2", "ME6")
...
# Genetic correlation posterior summary
rGN <- G12N / sqrt(G11N * G22N)
c(mean = mean(rGN),
  lwr = quantile(rGN, 0.025),
  upr = quantile(rGN, 0.975))

rGU <- G12U / sqrt(G11U * G22U)
c(mean = mean(rGU),
  lwr = quantile(rGU, 0.025),
  upr = quantile(rGU, 0.975))
...
ggplot(post_longN, aes(x = Posterior)) +
...
ggplot(post_longU, aes(x = Posterior)) +
  geom_density(fill = "steelblue", alpha = 0.6) +
  facet_wrap(~Parameter, scales = "free", ncol = 2) +
  theme_classic(base_size = 14) +
  labs(
...
dat$par <- substr(as.character(dat$animal), 
                  nchar(as.character(dat$animal)), 
                  nchar(as.character(dat$animal)))

# brms version
brms_g <- brm(bf(ME2 ~ (par | gr(animal, cov = cstm_A_mat))) +
                       bf(ME6 ~ (par | gr(animal, cov = cstm_A_mat))) +
                       set_rescor(TRUE),
                data = dat,
...
post <- as_draws_df(brms_g)

save(post, brms_g, file = "brm_ME2_ME6_fit_posts.RData")

# N group (reference level)
me2_N_cols <- grep(
  "^r_animal__ME2\\[.*,Intercept\\]$",
  colnames(post),
  value = TRUE
)

me6_N_cols <- grep(
  "^r_animal__ME6\\[.*,Intercept\\]$",
  colnames(post),
  value = TRUE
)
...
# Ensure same animal ordering
animals_N_me2 <- sub("r_animal__ME2\\[(.*),Intercept\\]", "\\1", me2_N_cols)
animals_N_me6 <- sub("r_animal__ME6\\[(.*),Intercept\\]", "\\1", me6_N_cols)

animals_U_me2 <- sub("r_animal__ME2\\[(.*),parU\\]", "\\1", me2_Udev_cols)
animals_U_me6 <- sub("r_animal__ME6\\[(.*),parU\\]", "\\1", me6_Udev_cols)

stopifnot(all(animals_N_me2 == animals_N_me6))
stopifnot(all(animals_U_me2 == animals_U_me6))
stopifnot(all(animals_N_me2 == animals_U_me2))

# Storage
Vg_ME2_N <- Vg_ME6_N <- Covg_N <- rG_N <- numeric(nrow(post))
Vg_ME2_U <- Vg_ME6_U <- Covg_U <- rG_U <- numeric(nrow(post))

G_N <- vector("list", nrow(post))
G_U <- vector("list", nrow(post))

for(i in seq_len(nrow(post))) {

  #### N group ####
  bv_me2_N <- as.numeric(post[i, me2_N_cols])
  bv_me6_N <- as.numeric(post[i, me6_N_cols])

  Vg_ME2_N[i] <- var(bv_me2_N)
  Vg_ME6_N[i] <- var(bv_me6_N)

  Covg_N[i] <- cov(bv_me2_N, bv_me6_N)

  rG_N[i] <- cor(bv_me2_N, bv_me6_N)
  ...
  G_N[[i]] <- matrix(
    ...
    nrow = 2,
    byrow = TRUE,
    dimnames = list(c("ME2", "ME6"),
                    c("ME2", "ME6"))
  )

  #### U group ####
  # U = Intercept + parU deviation
  bv_me2_U <- as.numeric(post[i, me2_N_cols]) +
              as.numeric(post[i, me2_Udev_cols])

  bv_me6_U <- as.numeric(post[i, me6_N_cols]) +
              as.numeric(post[i, me6_Udev_cols])

  Vg_ME2_U[i] <- var(bv_me2_U)
  Vg_ME6_U[i] <- var(bv_me6_U)

  Covg_U[i] <- cov(bv_me2_U, bv_me6_U)

  rG_U[i] <- cor(bv_me2_U, bv_me6_U)
...
G_mean_N <- Reduce("+", G_N) / length(G_N)
G_mean_U <- Reduce("+", G_U) / length(G_U)
